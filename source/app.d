...
int main(string[] args) {
	...
	GetoptResult reslt_opt = getopt(
		args,
		"init","Initialize a mgit instance from a manifest file" , &init
	);

	if(reslt_opt.helpWanted)
		defaultGetoptPrinter("", reslt_opt.options);

	if(!exists(getcwd()~"/.mgit")) writeln("Error: Not initialized");
	return 0;
}